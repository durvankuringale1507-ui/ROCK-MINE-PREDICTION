# ROCK-MINE-PREDICTION

import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.linear_model import LogisticRegression
from sklearn.metrics  import accuracy_score

#LOADING DATASET
dataset = pd.read_csv('/content/sonar.csv', header = None)
dataset.head()

dataset.shape

dataset.describe()

TOTAL MEAN AND ROCKS


dataset[60].value_counts()

M = MINE AND R = ROCK

dataset.groupby(60).mean()

#SEPRATING DATA INTO LABELS
x = dataset.drop(columns = 60, axis = 1)
y = dataset[60]
print(x,y)

x_train,x_test,y_train,y_test = train_test_split(x,y,test_size = 0.2,stratify = y, random_state = 2)      #STRATIFY } equal mines and rocks in training data
print(x.shape,x_train.shape,x_test.shape)
print(x_train)
print(y_train)

#MODEL TRAINING & LOGISTIC REGRESSION
model = LogisticRegression()
model.fit(x_train,y_train)

#MODEL EVALUATION
#accuracy on train data
pred = model.predict(x_train)
data_acc = accuracy_score(pred,y_train)
print("Accuracy On Training Data : ",data_acc)

#MODEL EVALUATION
#accuracy on test data
pred = model.predict(x_test)
data_acc = accuracy_score(pred,y_test)
print("Accuracy On Test Data : ",data_acc)

#Predictive System
input = (0.0307,0.0523,0.0653,0.0521,0.0611,0.0577,0.0665,0.0664,0.1460,0.2792,0.3877,0.4992,0.4981,0.4972,0.5607,0.7339,0.8230,0.9173,0.9975,0.9911,0.8240,0.6498,0.5980,0.4862,0.3150,0.1543,0.0989,0.0284,0.1008,0.2636,0.2694,0.2930,0.2925,0.3998,0.3660,0.3172,0.4609,0.4374,0.1820,0.3376,0.6202,0.4448,0.1863,0.1420,0.0589,0.0576,0.0672,0.0269,0.0245,0.0190,0.0063,0.0321,0.0189,0.0137,0.0277,0.0152,0.0052,0.0121,0.0124,0.0055)

#changing input to numpy
input_np = np.asarray(input)

#reshape input
input_reshape = input_np.reshape(1,-1)

#prediction
pred = model.predict(input_reshape)
print(pred)

if (pred[0] == 'R'):
  print("OBJECT IS ROCK")
else :
  print("OBEJCT IS MINE")
